...
- (void)mycopy:(id)sender {
    ...
    if ([_messageModel.type isEqualToString:KFDS_MESSAGE_TYPE_TEXT]) {
        [[UIPasteboard generalPasteboard] setString:_messageModel.content];
    } else if ([_messageModel.type isEqualToString:KFDS_MESSAGE_TYPE_IMAGE]){
...
//    NSLog(@"%s", __PRETTY_FUNCTION__);
    ...
    } else if ([_messageModel.type isEqualToString:KFDS_MESSAGE_TYPE_VOICE]) {
...
- (void)layoutTimestampLabel {
    //
    NSString *timestampString = [KFDSCUtils getOptimizedTimestamp:_messageModel.timestamp];
